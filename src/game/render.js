...
class Render {
  constructor(canvas, world, controller, controllerCallback) {
    this.canvas = canvas;
    this.world = world;
  ...
  doAnimate() {
    if (!this.paused) {
      ...
      if (!this.gameEnded && player.position.y > this.canvas.height) {
        this.gameEnded = true;
      ...
      // If the player is higher than the middle
      // of the canvas, push objects down.
